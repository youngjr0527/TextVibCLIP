...
# UOS 7-클래스: 0:H_H, 1:H_B, 2:H_IR, 3:H_OR, 4:L_H, 5:U_H, 6:M_H
UOS_CLASS_ID_TO_NAME = {0: "H", 1: "B", 2: "IR", 3: "OR", 4: "L", 5: "U", 6: "M"}
UOS_CLASS_ID_TO_TEXTS = {
    0: [
        "healthy bearing",
...
def find_latest_experiment_dir(results_dir: Path, scenario_name: str) -> Path:
    ...
    raise FileNotFoundError(
        f"No experiment dir with checkpoints/{scenario_name} under {results_dir}"
    )


def load_model_for_domain(ckpt_base: Path, domain_value: int, device: torch.device) -> torch.nn.Module:
    # 도메인별 best 체크포인트 우선, 없으면 first_domain_final.pth로 폴백(600RPM)
    candidates = [
    ...
    model = create_textvib_model(domain_stage="continual", dataset_type="uos").to(device)
...
def build_class_prototypes(model: torch.nn.Module, device: torch.device) -> torch.Tensor:
    # 각 클래스의 프롬프트 임베딩 평균 → 프로토타입 (7, dim)
    class_embs = []
    for cls_id in [0, 1, 2, 3, 4, 5, 6]:
        texts = UOS_CLASS_ID_TO_TEXTS[cls_id]
    ...
    return torch.cat(class_embs, dim=0)  # (7, dim)
...
def run_verify(experiment_dir: Path, samples_per_domain: int, device: torch.device, domains: List[int]) -> None:
    ...
    data_dir = Path(DATA_CONFIG["data_dir"])  # data_scenario1
    overall_correct = 0
    overall_total = 0
    ...
    for domain_value in domains:
        ...
        prototypes = build_class_prototypes(model, device)  # (7, dim)
        ...
        n = len(test_ds)
        if n == 0:
            print(f"[{domain_value}RPM] test samples = 0 (skip)")
            continue

        picked = sample_indices(n, samples_per_domain, seed=42 + domain_value)
        correct = 0

        print(f"\n=== Verify UOS {domain_value}RPM: {len(picked)}/{n} random samples ===")
        for idx in picked:
            sample = test_ds[idx]
            vib = sample["vibration"].unsqueeze(0).to(device)  # (1, 2048)
            lbl = sample["labels"]
            # UOS 라벨: tensor([main_label, bearing_type_label]) → main_label만 사용
            lbl_id = int(lbl[0].item())
            meta = sample.get("metadata", {})
            fpath = meta.get("filepath", "unknown")
            widx = int(sample.get("window_idx", -1))
            # 실제 텍스트 라벨(조합) 표시용
            rc = meta.get("rotating_component", "H")
            bc = meta.get("bearing_condition", "H")
            true_combo = f"{rc}_{bc}"

            with torch.no_grad():
                vib_raw = model.vib_encoder(vib)
                vib_emb = F.normalize(model.vib_projection(vib_raw), p=2, dim=1)  # (1, dim)
                sims = torch.matmul(vib_emb, prototypes.t()).squeeze(0)  # (7,)
            ...
            topk_str = ", ".join(
                [
                    f"{UOS_CLASS_ID_TO_NAME[int(cid)]}:{float(v):.3f}"
                    for v, cid in zip(topk_vals.tolist(), topk_ids.tolist())
                ]
            )
            print(
                f"- file={os.path.basename(fpath)} win={widx:>4} | true={true_combo}({UOS_CLASS_ID_TO_NAME[lbl_id]}) "
                f"pred={UOS_CLASS_ID_TO_NAME[pred_id]} | top3[{topk_str}]"
            )

        acc = 100.0 * correct / max(1, len(picked))
        print(f"=== Domain {domain_value}RPM summary: {correct}/{len(picked)} ({acc:.2f}%) ===")

    if overall_total > 0:
        print(
            f"\n>>> Overall sampled accuracy: {overall_correct}/{overall_total} "
            f"({100.0*overall_correct/overall_total:.2f}%)"
        )


def main():
    parser = argparse.ArgumentParser(description="Verify random UOS test samples via retrieval (7-class)")
    parser.add_argument("--experiment_dir", type=str, default="", help="results/{timestamp} 디렉토리 경로")
    parser.add_argument("--samples_per_domain", type=int, default=30)
    parser.add_argument(
        "--domains",
        type=str,
        default="600,800,1000,1200,1400,1600",
        help="검증할 도메인 리스트. 예: 600,800,1000,1200,1400,1600",
    )
    ...
    if args.experiment_dir:
        ...
        experiment_dir = find_latest_experiment_dir(results_dir, scenario_name)
